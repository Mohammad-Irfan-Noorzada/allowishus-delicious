      ...
      <section id="alowishus" className="relative mt-12 pt-12 px-4 lg:pt-28 bg-gradient-to-t to-neutral-ultraLightGray from-neutral-veryLightGray rounded-e-[4rem]
        ...
        <div className="text-center sm:max-w-[45rem] mx-auto space-y-3">
          ...
          <p className="sm:text-lg lg:text-[1.3rem] xl:text-2xl text-neutral-dimGray sm:px-10">
            Enjoy a refreshing cup from our signature cold brew collection OR try one of our expertly crafted espresso drinks.
          </p>
        </div>

        <div className="grid sm:grid-cols-2 lg:grid-cols-3 mx-auto gap-6 mt-12 md:mt-14 xl:mt-24">
          {allowishuses.map((allowishus, id) => (
            <div key={id} className="bg-neutral-white py-7 px-5 md:px-8 md:py-10 rounded-2xl">
              <div className="space-y-2 md:space-y-5">
                <h3 className="text-[1.3rem] md:text-2xl lg:text-[1.8rem] xl:text-[2.25rem] font-semibold text-neutral-veryDarkGray">{allowishus.title}</h3>
                <p className="text-[1.1rem] lg:text-xl xl:text-2xl text-neutral-dimGray">{allowishus.text}</p>
              </div>
              ...
              <div className="flex flex-col items-center mt-10 gap-10 md:gap-14">
                ...
                />
                <button className="uppercase py-4 lg:px-6 xl:px-7 rounded-[2.5rem] max-w-64 md:max-w-44 lg:max-w-52 xl:max-w-56 w-full 
